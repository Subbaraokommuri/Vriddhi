...
export function runMigrations(db: Database.Database) {
  ...
  if (v1Exists) {
    ...
    `);
    db.exec('DROP TABLE nav_history_v1');
  }
  ...
  if (bpExists) {
    ...
    `);
    db.exec('DROP TABLE benchmark_prices');
  }

  // Retired portfolio feature — tables and the legacy portfolio_folios are gone
  db.exec(`
  ...
  const tablesToCheck = ['folios', 'transactions', 'funds'];
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS folio_tags (
      ...
      PRIMARY KEY (folio_id, tag)
    );

    CREATE TABLE IF NOT EXISTS theme_tags (
      theme_id  TEXT NOT NULL REFERENCES tag_themes(id),
      tag       TEXT NOT NULL,
  ...
  const ubCols = db.prepare('PRAGMA table_info(user_benchmarks)').all() as any[];
  if (!ubCols.find((c: any) => c.name === 'benchmark_type')) {
    db.exec("ALTER TABLE user_benchmarks ADD COLUMN benchmark_type TEXT DEFAULT 'yahoo'");
  }
  if (!ubCols.find((c: any) => c.name === 'amfi_code')) {
